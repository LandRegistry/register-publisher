...
class Application(object):
    ...
    def start(self):
        ...
        logging.debug("Starting '{}' as a separate process.".format(self.target))
        self.process.start()

    def join(self, timeout=None):
        self.process.join(timeout=timeout)
        logging.debug("'{}' completed.".format(self.target))

    def terminate(self):
        self.process.terminate()
        logging.debug("'{}' terminated.".format(self.target))
...
class TestRegisterPublisher(unittest.TestCase):
    ...
    def handle_message(self, body, message):
        ...
        logging.info('Received message: {}'.format(message.payload))
        logging.info(' properties: {}'.format(message.properties))
        logging.info(' delivery_info: {}'.format(message.delivery_info))
        message.ack()
    ...
    def consume(self, cfg=server.incoming_cfg):
        ...
        with server.setup_consumer(cfg=cfg, callback=self.handle_message) as consumer:
            ...
            logging.info("'drain_events()' with timeout")
            try:
    ...
    def test_incoming_queue(self):
        ...
        logging.info("Put message, exchange: {}, {}".format(self.message, producer.exchange))
